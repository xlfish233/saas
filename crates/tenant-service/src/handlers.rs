...
pub async fn list_tenants(
    ...
    let tenants = state.tenant_service
        .list_tenants()
        .await
        .map_err(|e| {
...
pub async fn create_tenant(
    ...
    let isolation = shared::tenant::IsolationLevel::from_str(&req.isolation_level)
        .map_err(|_| (StatusCode::BAD_REQUEST, Json(ErrorResponse::new("Invalid isolation level"))))?;

    let plan = shared::tenant::Plan::from_str(&req.plan)
        .map_err(|_| (StatusCode::BAD_REQUEST, Json(ErrorResponse::new("Invalid plan"))))?;
    ...
    let tenant = state.tenant_service
        ...
        .map_err(|e| {
            ...
            match e.downcast_ref::<sqlx::Error>() {
                Some(sqlx::Error::Database(db_err)) if db_err.constraint() == Some("tenants_slug_key") => {
                    (StatusCode::CONFLICT, Json(ErrorResponse::new("Tenant slug already exists")))
                }
                _ => (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse::new("Internal error"))),
            }
        })?;
...
pub async fn get_tenant(
    ...
    let tenant = state.tenant_service
        .get_tenant(id)
        .await
        .map_err(|_| (StatusCode::NOT_FOUND, Json(ErrorResponse::new("Tenant not found"))))?;
...
pub async fn create_schema(
    ...
    let schema_name = state.tenant_service
        .create_tenant_schema(id)
        .await
        .map_err(|e| {
            tracing::error!("Failed to create schema: {}", e);
            (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse::new("Failed to create schema")))
        })?;
